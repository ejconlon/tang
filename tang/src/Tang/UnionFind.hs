...
import Optics (Lens')
import Tang.Util (execStateLens, runStateLens)
...
members u@(UnionFind _ p) = foldr go (ILM.empty, u) (ILM.keys p)
 where
  go x1 (m, v) =
...
findRoot u@(UnionFind z p) x1 =
  let (acc, x2) = findRootAcc p [] x1
      u' = case acc of
        [] -> u
        _ ->
          let p' = foldr (`ILM.insert` x2) p acc
          in  UnionFind z p'
  in  (x2, u')
...
merge i j u@(UnionFind z p) = guardMerge
 where
  guardMerge =
    if ILM.member i p
      then
    ...
    in  if ix2 == jx2
          then
            let u' = case acc of
                  [] -> u
                  _ ->
                    let p' = foldr (`ILM.insert` ix2) p acc
                    in  UnionFind z p'
